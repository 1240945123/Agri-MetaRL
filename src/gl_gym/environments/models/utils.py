...
def define_model(
    ...
    n_params: int,
    dt: float,
    integrator_options: Mapping[str, Any] | None = None,
):
    """
    ...
    Args:
        ...
        integrator_options (Mapping[str, Any] | None): Diagnostic-only CVODES
            option overrides. Formal environment runs use the immutable defaults.
    ...
    int_opts = dict(FORMAL_CVODES_OPTIONS)
    if integrator_options is not None:
        int_opts.update(integrator_options)
    F = ca.integrator(
        "F", "cvodes",

...
from gl_gym.paths import CONFIG_DIR, MODEL_DIR
...
class ExperimentManager:
    ...
    def __init__(
        ...
        self.continue_vecnormalize_path = continue_vecnormalize_path
        ...
        self.hyp_config_path = str(CONFIG_DIR / "sweeps")
    ...
    def artifact_run_name(self):
        """Return the stable run directory name used for saved artifacts."""

        return self.run_name or self.run.name
    ...
    def init_envs(self, gamma):
        ...
        if self.continue_vecnormalize_path is not None and os.path.isfile(self.continue_vecnormalize_path):
            self.env = VecNormalize.load(self.continue_vecnormalize_path, self.env)
        ...
        if self.continue_vecnormalize_path is not None and os.path.isfile(self.continue_vecnormalize_path):
            self.eval_env = VecNormalize.load(self.continue_vecnormalize_path, self.eval_env)
    ...
    def initialise_model(self):
        ...
        artifact_run_name = self.artifact_run_name()
        if self.stochastic:
            tensorboard_log = str(MODEL_DIR / self.project / self.algorithm / "stochastic" / "logs" / artifact_run_name)
        else:
            tensorboard_log = str(MODEL_DIR / self.project / self.algorithm / "deterministic" / "logs" / artifact_run_name)

        if self.continue_model_path is not None and os.path.isfile(self.continue_model_path):
    ...
    def build_model_parameters(self):
        ...
        # AgriMetaRL: meta_* stay in model_params (passed to constructor), not in policy_kwargs
        if self.algorithm == "agri_metarl" and "policy_kwargs" in model_params:
            for key in list(model_params["policy_kwargs"].keys()):
                if key.startswith("meta_"):
                    del model_params["policy_kwargs"][key]
    ...
    def run_experiment(self):
        ...
        artifact_run_name = self.artifact_run_name()
        if self.stochastic:
            model_log_dir = str(MODEL_DIR / self.project / self.algorithm / "stochastic" / "models" / artifact_run_name) if self.save_model else None
            env_log_dir = str(MODEL_DIR / self.project / self.algorithm / "stochastic" / "envs" / artifact_run_name) if self.save_env else None
        else:
            model_log_dir = str(MODEL_DIR / self.project / self.algorithm / "deterministic" / "models" / artifact_run_name) if self.save_model else None
            env_log_dir = str(MODEL_DIR / self.project / self.algorithm / "deterministic" / "envs" / artifact_run_name) if self.save_env else None
...
def main():
    ...
    env_config_path = str(CONFIG_DIR / "envs")

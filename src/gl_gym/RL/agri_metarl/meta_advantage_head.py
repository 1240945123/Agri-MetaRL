...
class TransitionSetEncoder(nn.Module):
    """Encode an unordered support set of transitions into task context."""
    ...
    def __init__(
        ...
        super().__init__()
        transition_dim = obs_dim + action_dim + 1 + obs_dim + 1
    ...
    def forward(
        ...
        transitions = torch.cat(
            [
                observations,
                actions,
                rewards.to(observations.dtype).unsqueeze(-1),
                next_observations,
                dones.to(observations.dtype).unsqueeze(-1),
            ],
            dim=-1,
        )
        return self.transition_encoder(transitions).mean(dim=-2)


class AdvantageResidualHead(nn.Module):
    """Apply a bounded, context-conditioned residual to raw GAE values."""
    ...
    def __init__(
        ...
        self.residual_mlp = nn.Sequential(
            nn.Linear(context_dim * 2 + 1, hidden_dim),
            nn.SiLU(),
    ...
    def forward(
        ...
        projected_observation = self.observation_projection(observations)
        features = torch.cat(
            [projected_observation, raw_advantage.unsqueeze(-1), context], dim=-1
        )

...
OPTIMIZER = {"ADAM": Adam}
ENVS = {"TomatoEnv": TomatoEnv, "ShieldedTomatoEnv": ShieldedTomatoEnv}
...
def make_env(env_id, rank, seed, env_base_params, env_specific_params, eval_env):
    ...
    '''
    def _init():
        # eval_options_heldout is only for eval scripts to override eval_options; not a TomatoEnv __init__ arg
        spec = {k: v for k, v in env_specific_params.items() if k != "eval_options_heldout"}
        spec["environment_index"] = rank
        env = ENVS[env_id](**spec, base_env_params=env_base_params)
        # if not env.training:

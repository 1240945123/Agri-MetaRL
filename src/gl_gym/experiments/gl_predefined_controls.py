...
from gl_gym.paths import CONFIG_DIR, RESULT_DIR
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env_config_path", type=str, default=str(CONFIG_DIR / "envs"))
    ...
    controls = pd.read_csv(RESULT_DIR / 'AgriControl/comparison/matlab/controls_pipe2009.csv').values[:Ns_data, :6]
    weather = pd.read_csv(RESULT_DIR / 'AgriControl/comparison/matlab/weather_pipe2009.csv').values[:Ns_data, :]
    ...
    df.to_csv(RESULT_DIR / "AgriControl/run_times/gl_gym.csv", index=False)
    ...
    if args.save_state:
        ...
        states.to_csv(RESULT_DIR / "AgriControl/comparison/gl_gym/states_pipe2009.csv", index=False)
        ...
        weather_data.to_csv(RESULT_DIR / "AgriControl/comparison/gl_gym/weather_pipe2009.csv", index=False)

        controls_cols = ["uBoil", "uCO2", "uThScr", "uVent", "uLamp", "uBlScr"]
        controls_data = pd.DataFrame(controls, columns=controls_cols)
        controls_data.to_csv(RESULT_DIR / "AgriControl/comparison/gl_gym/controls_pipe2009.csv", index=False)

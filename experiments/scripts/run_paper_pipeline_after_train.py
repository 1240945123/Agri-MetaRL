...
Call after train_paper_experiments.py has produced models under artifacts/models/AgriControl/<algo>/deterministic/.
"""
import os
import sys
import subprocess
...
os.chdir(ROOT)
SCRIPTS_DIR = Path(ROOT) / "experiments" / "scripts"
...
def main():
    ...
    parser.add_argument("--model_root", type=str, default="artifacts/models/AgriControl")
    parser.add_argument("--out_dir", type=str, default="artifacts/results/AgriControl")
    parser.add_argument("--n_episodes", type=int, default=3, help="Episodes per eval (reduce for speed)")
    args = parser.parse_args()

    if not args.skip_train:
        run([sys.executable, script("train_paper_experiments.py"), "--device", "cpu"], "1. Train (PPO, RecurrentPPO, Agri-MetaRL, 5 seeds)")
    if args.train_ablation:
        run([sys.executable, script("train_ablation_variants.py"), "--device", "cpu"], "1b. Train ablation variants (no_bn, no_clip, state_context_only, advantage_only)")

    run([sys.executable, script("run_all_fixed_protocol.py")], "2. Fixed protocol (all algorithms)")

    # 2b. Record 60d trajectory for long_horizon figure
    subprocess.run(
        [sys.executable, script("record_trajectory_60d.py"), "--algorithm", "agri_metarl"],
        cwd=ROOT
    )

    # 2c. Export learning curves from wandb
    subprocess.run(
        [sys.executable, script("export_learning_curves.py"), "--project", "AgriControl"],
        cwd=ROOT
    )
    ...
    for algo in ["ppo", "recurrentppo", "agri_metarl"]:
        ...
        if not os.path.isdir(os.path.join(model_root, "models")):
            continue
        r = subprocess.run(
            [sys.executable, script("evaluate_train_test_generalization.py"),
    ...
    subprocess.run(
        [sys.executable, script("evaluate_few_update.py"), "--n_episodes", str(args.n_episodes)],
        cwd=ROOT
    )
    ...
    ablation_base = os.path.join(ROOT, "artifacts", "models", "AgriControl_ablation", "agri_metarl", "deterministic")
    r = subprocess.run(
        [sys.executable, script("run_ablation.py"),
    ...
    if r.returncode == 0:
        ...
        if first_run_dir:
            subprocess.run(
                [sys.executable, script("run_ablation.py"), "--variant_dirs", f"full:{first_run_dir}",
                 "--n_episodes", str(args.n_episodes)],
                cwd=ROOT
            )

...
Output: artifacts/results/AgriControl/fixed_protocol/trajectory_24h.csv (method, step, temp_air, rh_air, co2_air, uBoil, uVent, uCo2)
...
def record_trajectory(algorithm, model_dir, env, n_steps=STEPS_24H):
    ...
    model_class = ALG_MAP[algorithm]
    model_path = os.path.join(model_dir, "best_model.zip") if model_dir else None
    model = model_class.load(model_path, device="cpu") if model_path and os.path.isfile(model_path) else None
...
def main():
    ...
    parser.add_argument("--out_path", type=str, default="artifacts/results/AgriControl/fixed_protocol/trajectory_24h.csv")

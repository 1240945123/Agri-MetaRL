...
N_STEPS = 5760
DATA_ROOT = os.path.join(ROOT, "artifacts", "results", "AgriControl")
...
PAPER = os.path.join(ROOT, "paper", "manuscript", "main.tex")
...
def main():
    ...
    for method in ["agri_metarl", "recurrentppo", "ppo", "rule_baseline"]:
        ...
        epi_str = f"${epi:.2f}$" if epi < 0 else (f"${epi:.2f} \\pm {epi_std:.2f}$" if epi_std > 0 else str(round(epi, 2)))
        train_row = {
            "name": name,
        ...
        if method == "rule_baseline":
            ...
            test_epi = f"${epi_t:.2f}$" if epi_t < 0 else (f"${epi_t:.2f} \\pm {epi_std_t:.2f}$" if epi_std_t > 0 else str(round(epi_t, 2)))
